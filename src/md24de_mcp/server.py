...
def _make_client() -> Md24deClient:
    ...
    return Md24deClient(
        ...
        options=ClientOptions(timeout=cfg.timeout),
    )
...
def _get_latest_report_pdf() -> bytes:
    ...
    _log.debug("Resource: rendering PDF locally from portal data")
    ...
    _log.debug("Resource: PDF rendered: %d bytes", len(pdf_bytes))
    return pdf_bytes
...
def save_pdf(directory: str = "~/Downloads") -> dict[str, object]:
    """Render the monthly consumption PDF locally and save it to disk.
    ...
    to provide to tenants under §6a HeizkostenV. The document is rendered locally from the
    portal's consumption data and covers the same month returned by get_last_available_month.
    ...
    if cached_pdf is not None and cached_month is not None:
        ...
        _log.debug("Rendering PDF locally from portal data")
        with _make_client() as client:
            month = client.get_last_available_month()
            pdf_bytes = client.get_pdf()
        caches.month.set(month)
        caches.pdf.set(pdf_bytes)
        _log.debug("PDF rendered: %d bytes for %04d-%02d", len(pdf_bytes), month.year, month.month)
